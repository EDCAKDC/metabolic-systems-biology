...
# Helper: extract true ID from frozenset like {'GAPD'}


def extract_single_id(x):
    """
    Extract a single ID from the 'ids' column produced by COBRA
    single_reaction_deletion / single_gene_deletion.
    """
...
# IMPORTANT: extract real reaction IDs from rxn_del["ids"]
rxn_del["reaction_id"] = rxn_del["ids"].apply(extract_single_id)
...
# Mapping: reaction_id → essentiality_class
...
# Use the same idea: extract real gene IDs from g_del["ids"]
if "ids" in g_del.columns:
    g_del["gene_id"] = g_del["ids"].apply(extract_single_id)
else:
...
# Mapping: gene_id → essentiality_class
...
color_map = {
    ...
    "partially_essential": "#ff7f0e",  # orange
    "non_essential": "#2ca02c",        # green
    "unknown": "#7f7f7f",              # gray
}
...
# Debug: print a few nodes to confirm essentiality values
print("Sample nodes in H after annotation:")
for n, data in list(H.nodes(data=True))[:10]:
    print(n, "->", data.get("essentiality"))
